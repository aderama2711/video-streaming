...
class Fields {
    constructor(...args) {
        ...
        this.lifetime_ = Interest.DefaultLifetime;
...
export class Interest {
    ...
    encodeTo(encoder) {
        ...
        encoder.prependTlv(TT.Interest, f.name, f.canBePrefix ? [TT.CanBePrefix] : undefined, f.mustBeFresh ? [TT.MustBeFresh] : undefined, f.fwHint, [TT.Nonce, NNI(f.nonce ?? Interest.generateNonce(), { len: 4 })], f.lifetime === Interest.DefaultLifetime ?
            ...
            undefined : [TT.HopLimit, NNI(f.hopLimit, { len: 1 })], ...this.encodeParamsPortion());
    }
    encodeParamsPortion() {
        if (!this.appParameters) {
            return [];
        }
        const w = [[TT.AppParameters, this.appParameters]];
        if (this.sigInfo) {
            w.push(this.sigInfo.encodeAs(TT.ISigInfo), [TT.ISigValue, this.sigValue]);
        }
        return w;
    }
